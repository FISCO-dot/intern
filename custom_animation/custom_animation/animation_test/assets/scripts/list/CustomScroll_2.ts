...
export default class List extends cc.Component {
    ...
    @property({
        ...
        visible(){
            if((this.cycle&&this.scrollVertical)||this.messageMode) this.scrollHorizontal = false;
    ...
    private _calculatePosition(item:any){
        if(this._itemPosition[item.name]) return this._itemPosition[item.name]
        else{
            if(!this.adaptiveSize){
                for(var i = 0;i < this._data.length;i++){
                    let column = i / this.itemNumX
                    let row = i % this.itemNumX
                    this._itemPosition[String[i]] = cc.v2(this.width/2+row*(this.width+this.interval),this.height/2+column*(this.height+this.interval))
                }
            }
            else{
                if(this.scrollHorizontal){
